...
export default function OverviewDashboard({
  ...
  const quickLinks = [
    { href: `${basePath}/alumni`, label: "Approve alumni", detail: "47 waiting" },
    { href: `${basePath}/news`, label: "Publish news", detail: "Create post" },
    { href: `${basePath}/events`, label: "Schedule event", detail: "Calendar" },
    { href: `${basePath}/messages`, label: "Inbox", detail: "9 unread" },
  ];
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
          ...
          <Link
            href={`${basePath}/news`}
            className="inline-flex h-11 w-full items-center justify-center border border-unn-line bg-white px-4 text-sm font-semibold text-unn-ink transition hover:border-unn-green sm:w-auto"
          >
            New announcement
          </Link>
        </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.4fr_0.9fr]">
        <div className="border border-unn-line bg-white">
          <div className="flex flex-col gap-2 border-b border-unn-line px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div>
          ...
          <ul className="divide-y divide-unn-line md:hidden">
            {recentAlumni.map((person) => (
              <li key={person.name} className="px-4 py-3.5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-medium text-unn-ink">{person.name}</p>
                    <p className="mt-1 text-sm text-unn-muted">
                      {person.faculty} · Class of {person.year}
                    </p>
                  </div>
